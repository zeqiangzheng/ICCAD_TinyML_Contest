...
from util import *
from models.model_vgg_test import vgg0959
...
def main(net):
    ...
    file_complete = creating_path("Result","logs", args.model,
                                  file_name=args.model + args.comment + str(args.batch_size) + args.loss, extension='log')
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times (specify the #epoch)
        ...
        for j, data in enumerate(trainloader):
            ...
            #inputs = torch.abs(torch.fft.fft(inputs))
            inputs = inputs.float().to(device)
            #labels = torch.abs(torch.fft.fft(labels))
        ...
        with torch.no_grad():
            for data_test in testloader:
                ...
                #IEGM_test = torch.abs(torch.fft.fft(IEGM_test))
        ...
        if best_f_beta_loss > Test_loss:
            ...
            torch.save(net, './saved_models/' + args.model + args.comment + str(args.batch_size) + args.loss + '.pkl')
            torch.save(net.state_dict(), './saved_models/' + args.model + args.comment + str(args.batch_size)  + args.loss + '_state_dict.pkl')
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--model', type=str, default='vgg0959')
    argparser.add_argument('--loss', type=str, default='f_beta')
    argparser.add_argument('--comment', type=str, default='None')
    args = argparser.parse_args()

    model_dic = {
    #"IEGM_net": IEGMNet(),
    #"mobilenetv2_simple": mobilenetv2(num_classes=2, width_mult=1.,last_channel = 160),
    "vgg0959": vgg0959()
    }
